...
function activate(context) {
    ...
    FILE_ACTIONS.forEach(function (action) {
        var disposable = vscode.commands.registerCommand("file tortoise-svn " + action + " mirror", function () {
            var path = vscode.window.activeTextEditor && vscode.window.activeTextEditor.document.uri.fsPath;
            ...
            if (!mirror) {
                ...
                var arquivo = vscode.window.activeTextEditor.document.uri.fsPath;
                var firstSep = arquivo.indexOf("\\") + 1;
                tortoiseCommand.exec(action, mirror + arquivo.substring(firstSep));
            }
        });
        context.subscriptions.push(disposable);
    });
...
var UriInfo = (function () {
    ...
    UriInfo.prototype.getActionQuickPickItem = function () {
        ...
        //if (this.isFile) {
            var mirror = vscode.workspace.getConfiguration('TortoiseSVN').get('mirrorRepository').toString();
            var firstSep = _this.path.indexOf("\\") + 1;
            if (mirror) {
                actions.forEach(function (action) {
                    quickPickItems.push({
